...
FORWARD _PROTOTYPE( int do_getprigroup, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_changeprigroup, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_setscheduler, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_getusagetime, (message *m_ptr) ); 
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_GETPRIGROUP: r = do_getprigroup(&m); break;
  	    case SYS_CHANGEPRIGROUP: r = do_changeprigroup(&m); break;
	    case SYS_SETSCHEDULER: r = do_setscheduler(&m); break;
	    case SYS_GETUSAGETIME: r = do_getusagetime(&m); break;
	    default:		r = E_BAD_FCN;
	}
...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  rpc->pri_group = m_ptr->PID % 2;  /* set pri_group based on process pid */
  rpc->cycles = rpc->pri_group % 2 ? 100 - scheduler_divider : scheduler_divider;
  return(OK);
}
...
PRIVATE int do_getprigroup(m_ptr)
register message *m_ptr;
{
	int i = 0;
	int arg = m_ptr->m2_l2;	
 ...
 *				do_changeprigroup			    *
 *==========================================================================*/
PRIVATE int do_changeprigroup(m_ptr)
register message *m_ptr;
{
	int i = 0;
	int caller_pid = m_ptr->m2_l2;		
	for(i = 0; i < NR_PROCS; i++){
		if(proc[i].p_pid == caller_pid){
			if(proc[i].pri_group == 0){
				proc[i].pri_group = 1;
				return 1;
			}
			else{
...
register message *m_ptr;
{
	...
	int arg = m_ptr->m2_l1;
	int caller_pid = m_ptr->m2_l2;	
	printf("previous scheduler divider is equal %d\n", scheduler_divider);	
	for(i = 0; i < NR_PROCS; i++){
		if(proc[i].p_pid == caller_pid){
			if(proc[i].pri_group == 0){
				if(arg >= 100) {
					printf("scheduler divider is equal %d\n", scheduler_divider);	
					return -3;
				}
				if(arg <= 0){
					printf("scheduler divider is equal %d\n", scheduler_divider);
 ...
 *				do_getusagetime			    *
 *==========================================================================*/
PRIVATE int do_getusagetime(m_ptr)
register message *m_ptr;
{
	int i = 0;
	int caller_pid = m_ptr->m2_l2;
