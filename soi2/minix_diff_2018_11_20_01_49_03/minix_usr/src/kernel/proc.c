 ...
 */
  int a_pri = scheduler_divider;
  int b_pri = 100 - scheduler_divider;
  int current_pri = 0;
  register struct proc *act;
  static int a_history = 0;
  static int b_history = 0;
  static int sum = 0;
 ...
 /* 
  ...
  while(1){
    ...
    current_pri = rdy_head[USER_Q]->pri_group;
    if(current_pri == 1 && b_history > b_pri) continue;
    if(current_pri == 0 && a_history > a_pri) continue;
    if(current_pri) ++b_history;
    else ++a_history;
    break;
  } 
  /*rdy_head[USER_Q]->cycles = rdy_head[USER_Q]->pri_group == 0 ? a_pri : b_pri;*/
  pick_proc();

/*
  register struct proc *temp, *prev;
  ...
  if(temp->p_type == SPRINT) {
    if(temp->cycles > 0) {
      ...
      temp->p_type = P_SPRINT;
    }
  }
  if(temp->p_type == P_SPRINT) {
    if(temp->cycles == 0) {
      temp->cycles = temp->cycles_base;
      prev = temp = rdy_head[USER_Q];
      while(prev->p_nextready!=NIL_PROC 
      && prev->p_nextready->p_type < CALC) {
        prev = prev->p_nextready;
      }
      if(prev==rdy_head[USER_Q]);
      else {
        temp = prev->p_nextready;
        prev->p_nextready = rdy_head[USER_Q];
        rdy_head[USER_Q] = rdy_head[USER_Q]->p_nextready;
        prev->p_nextready->p_nextready = temp;
      }
    }
    else {
      --(temp->cycles);
