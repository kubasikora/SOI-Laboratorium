...
PUBLIC int do_getprigroup(){
  message msg = mm_in;
  int caller_pid = mproc[mm_in.m_source].mp_pid;
  msg.m2_l2 = caller_pid;
  return _taskcall(SYSTASK, SYS_GETPRIGROUP, &msg);
}
...
PUBLIC int do_changeprigroup(){
  message msg = mm_in;
  int caller_pid = mproc[mm_in.m_source].mp_pid;
  msg.m2_l2 = caller_pid;
  return _taskcall(SYSTASK, SYS_CHANGEPRIGROUP, &msg);
}
...
*				get_usagetime				      *
*============================================================================*/
PUBLIC int get_usagetime(){
  message msg = mm_in;
  int caller_pid = mproc[mm_in.m_source].mp_pid;
  msg.m2_l2 = caller_pid;
  return _taskcall(SYSTASK, SYS_GETUSAGETIME, &msg);
}
